...
export function MemoryCard({ mem, index = 0 }: Props) {
    ...
    >
      <Tilt3D>
        <div className="bg-surface border border-primary/40 rounded-xl p-5 hover:border-primary hover:shadow-lg hover:shadow-primary/10 transition-all duration-300">
          ...
          <h3 className="text-sm font-bold text-text leading-tight mb-2 line-clamp-1">{mem.name}</h3>
          <p className="text-xs text-text-dim mb-3 line-clamp-2">{mem.description}</p>

          <div className="space-y-1 mb-4">
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">Type</span>
              <span className="text-text-muted font-mono text-[11px]">{mem.specs.type}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">Capacity</span>
              <span className="text-text-muted font-mono text-[11px]">{mem.specs.capacity}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">Speed</span>
              <span className="text-text-muted font-mono text-[11px]">{mem.specs.speed}</span>
            </div>
          </div>
