...
import {
  Check, ArrowLeft, ArrowRight, Network, Zap, Layers, Gauge, Server, Cpu,
} from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-bg-dark pt-[72px] lg:pt-[104px] pb-20 flex items-center justify-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-text-muted text-sm mb-8">The networking product you are looking for does not exist or may have been removed.</p>
      ...
      <div className="max-w-7xl mx-auto px-4">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-12">
          <motion.nav variants={itemVariants} className="flex items-center gap-2 text-sm">
            ...
            <span className="text-text-dim">/</span>
            <span className="text-text-muted truncate max-w-[200px]">{net.name}</span>
          </motion.nav>
            ...
            <div>
              <div className="relative h-[380px] md:h-[480px] rounded-2xl border border-border bg-surface overflow-hidden">
                ...
                <div className="absolute top-4 right-4 px-3 py-1.5 rounded-lg border border-primary/30 bg-surface/90 backdrop-blur-sm">
                  ...
                  <div className="text-sm font-bold text-primary font-mono">{net.specs.type}</div>
                </div>
                <div className="absolute bottom-4 left-4 px-3 py-1.5 rounded-lg border border-secondary/30 bg-surface/90 backdrop-blur-sm">
                  ...
                  <div className="text-sm font-bold text-secondary font-mono">{net.series}</div>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 mt-4">
                <div className="rounded-xl border border-border bg-surface p-3 md:p-4">
                  ...
                  <div className="text-xs md:text-sm font-mono font-bold text-text truncate">{net.specs.speed}</div>
                </div>
                <div className="rounded-xl border border-border bg-surface p-3 md:p-4">
                  <div className="text-[10px] font-mono text-text-dim uppercase tracking-wider mb-1">Ports</div>
                  <div className="text-xs md:text-sm font-mono font-bold text-text truncate">{net.specs.ports}</div>
                </div>
                <div className="rounded-xl border border-border bg-surface p-3 md:p-4">
                  <div className="text-[10px] font-mono text-text-dim uppercase tracking-wider mb-1">Form Factor</div>
                  <div className="text-xs md:text-sm font-mono font-bold text-text truncate">{net.specs.formFactor}</div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-xs font-mono text-text-dim">Best for: {net.bestFor}</span>
                </div>
                <h1 className="text-3xl lg:text-4xl font-black text-text leading-tight">{net.name}</h1>
              ...
              <p className="text-text-muted leading-relaxed">{net.longDescription}</p>
          ...
          <motion.div variants={itemVariants}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">Complete technical specifications for the {net.series}.</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {[
                { title: "Type", icon: Network, rows: [{ label: "Type", key: "type" as const }] },
                { title: "Performance", icon: Gauge, rows: [{ label: "Speed", key: "speed" as const }, { label: "Ports", key: "ports" as const }] },
                { title: "Physical", icon: Server, rows: [{ label: "Form Factor", key: "formFactor" as const }] },
                { title: "Management", icon: Cpu, rows: [{ label: "Management", key: "management" as const }] },
              ].map((group) => (
          ...
          <motion.div variants={itemVariants}>
            <div className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/10 via-secondary/5 to-transparent p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center gap-6 justify-between">
              <div>
                <h3 className="text-2xl md:text-3xl font-black text-text mb-2">Ready to deploy the {net.series}?</h3>
