    ...
    >
      <Tilt3D>
        <div className="bg-surface border border-primary/40 rounded-xl p-5 hover:border-primary hover:shadow-lg hover:shadow-primary/10 transition-all duration-300">
          ...
          <h3 className="text-sm font-bold text-text leading-tight mb-2 line-clamp-1">{server.name}</h3>
          <p className="text-xs text-text-dim mb-3 line-clamp-2">{server.description}</p>

          <div className="space-y-1 mb-4">
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">Form Factor</span>
              <span className="text-text-muted font-mono text-[11px]">{server.formFactor}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">GPU</span>
              <span className="text-text-muted font-mono text-[11px]">{server.gpuSupport}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-text-dim">CPU</span>
              <span className="text-text-muted font-mono text-[11px]">{server.cpuOptions}</span>
            </div>
          </div>
