...
import {
  Check, ArrowLeft, ArrowRight, Cpu, Server,
...
const statusStyles: Record<ChipProduct["status"], { label: string; variant: "green" | "cyan" | "amber" | "purple" | "default" }> = {
    ...
    return (
      <div className="min-h-screen bg-bg-dark pt-[72px] lg:pt-[104px] pb-20 flex items-center justify-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-text-muted text-sm mb-8">The chip you are looking for does not exist or may have been removed.</p>
          <Link href="/products">
            <Button variant="solid" icon={<ArrowLeft className="w-4 h-4" />}>Back to Products</Button>
          </Link>
        </motion.div>
      </div>
    );
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-12">
          ...
          <motion.div variants={itemVariants} className="grid lg:grid-cols-2 gap-8 items-start">
            <div>
              <div className="relative h-[380px] md:h-[480px] rounded-2xl border border-border bg-surface overflow-hidden">
                <div className="absolute inset-0"><ChipScene /></div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-xs text-text-dim font-mono uppercase tracking-wider px-2 py-0.5 rounded-full bg-surface-2 border border-border">{chip.series}</span>
                  <span className="text-xs text-text-dim">·</span>
                  <span className="text-xs font-mono text-text-dim">Best for: {chip.bestFor}</span>
                </div>
                <h1 className="text-3xl lg:text-4xl font-black text-text leading-tight">{chip.name}</h1>
              ...
              <p className="text-text-muted leading-relaxed">{chip.longDescription}</p>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                <Link href={`/comparison?add=${chip.slug}`}>
                  <Button variant="outline" size="lg" icon={<Layers className="w-4 h-4" />}>Add to Compare</Button>
                </Link>
                <Link href="/products">
          ...
          <motion.div variants={itemVariants}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">Complete technical specifications for the {chip.series}.</p>
              </div>
            </div>
          ...
          <motion.div variants={itemVariants}>
            <div className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/10 via-secondary/5 to-transparent p-8 md:p-10 flex flex-col md:flex-row items-start md:items-center gap-6 justify-between">
              <div>
                <h3 className="text-2xl md:text-3xl font-black text-text mb-2">Ready to deploy the {chip.series}?</h3>
          ...
          {relatedChips.length > 0 && (
            <motion.div variants={itemVariants}>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {relatedChips.map((related, index) => (
                  <ChipCard key={related.id} chip={related} index={index} />
                ))}
              </div>
            </motion.div>
          )}
