...
import { ArrowRight, Check, Cpu, Zap, HardDrive, Network, Layers, ShieldCheck, TrendingUp } from "lucide-react";
...
const LIGHTNING_SCENE = dynamic(
  () => import("@/components/scenes/LightningScene").then((mod) => mod.LightningScene),
  { ssr: false }
);

const ICON_MAP: Record<string, typeof Cpu> = {
  Cpu, Zap, HardDrive, Network, Layers, ShieldCheck, TrendingUp,
};

function FeatureCard({ feature, index }: { feature: TechnologyFeature; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group bg-bg-dark rounded-xl border border-border p-5 hover:border-primary/30 transition-all"
    >
