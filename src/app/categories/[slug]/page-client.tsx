...
import { ArrowLeft, Server, Cpu, Network, HardDrive, MemoryStick, Search, X } from "lucide-react";
...
function getCategoryIcon(iconName: string) {
  switch (iconName) {
    ...
    case "MemoryStick": return <MemoryStick className="w-5 h-5" />;
...
export default function CategoryDetailPage() {
  ...
  const categoryProducts = category
    ? ALL_PRODUCTS.filter((p) => "parentCategoryId" in p && p.parentCategoryId === category.id)
    : [];

  const grouped = useMemo(() => {
    const chips = categoryProducts.filter(isChip);
    const servers = categoryProducts.filter(isServer);
    const networking = categoryProducts.filter(isNetworking);
    const memory = categoryProducts.filter(isMemory);
    const storage = categoryProducts.filter(isStorage);
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          >
            ...
            <p className="text-text-muted text-sm mb-8 max-w-md">
              The product category you are looking for does not exist or may have been moved.
            </p>
            <Link href="/categories" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary-dark transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-8">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="text-2xl lg:text-3xl font-black text-text">{category.name}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            ...
            {Object.entries(grouped).filter(([, prods]) => prods.length > 0).map(([type]) => (
              <Badge key={type} variant="cyan" size="sm">
                {grouped[type as keyof typeof grouped].length} {TYPE_CONFIG[type]?.label || type}
              </Badge>
            ))}
          </div>
        </motion.div>
