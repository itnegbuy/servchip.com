...
const ChipViewerScene = dynamic(
  () => import("@/components/3d/ChipViewerScene").then((mod) => mod.ChipViewerScene),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-[500px] rounded-xl bg-gradient-to-br from-surface-2 to-bg-dark border border-border flex flex-col items-center justify-center gap-4">
        <div className="relative">
          <div className="w-24 h-20 bg-gradient-to-br from-surface to-surface-2 rounded-lg border border-primary/20 flex items-center justify-center">
            <Cpu className="w-10 h-10 text-primary/50" />
          </div>
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="w-1.5 h-3 bg-primary/30 rounded-sm" />
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 text-text-dim">
          ...
          <span className="text-[10px]">Drag to rotate ┬╖ Scroll to zoom</span>
        </div>
      </div>
    ),
  }
