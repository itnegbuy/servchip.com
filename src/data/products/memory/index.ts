import type { MemoryProduct } from "@/types";

export const SK_HYNIX_MEMORY: MemoryProduct[] = [
  {
    id: "sk-hynix-hbm3e",
    name: "SK hynix HBM3E",
    slug: "sk-hynix-hbm3e",
    manufacturer: "SK hynix",
    manufacturerId: "sk-hynix",
    series: "HBM3E",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-1.jpg"],
    description:
      "Industry-leading HBM3E memory with 1.18 TB/s bandwidth per stack for AI accelerators.",
    longDescription:
      "SK hynix HBM3E is the latest high-bandwidth memory designed for AI and HPC accelerators. Offering 1.18 TB/s bandwidth per stack with capacities up to 36GB per stack, it is the preferred memory for NVIDIA, AMD, and other AI accelerator vendors. Built on advanced 1b nm process technology, it delivers the highest performance for AI training and inference.",
    keyFeatures: [
      "1.18 TB/s Bandwidth per Stack",
      "Up to 36GB Capacity",
      "1b nm Process",
      "8-Hi / 12-Hi Stacks",
      "TSV Technology",
    ],
    useCases: ["ai-training", "ai-inference", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3E",
      capacity: "36GB per stack",
      speed: "9.6 Gbps",
      formFactor: "8-Hi / 12-Hi Stack",
      voltage: "1.1V",
      bandwidth: "1.18 TB/s per stack",
    },
    status: "in_stock",
    sortOrder: 1,
    isFeatured: true,
    isPopular: true,
    seo: {
      metaTitle: "SK hynix HBM3E | Servchip",
      metaDescription: "Industry-leading HBM3E memory for AI accelerators.",
    },
    createdAt: "2024-06-01",
    updatedAt: "2025-01-01",
  },
  {
    id: "sk-hynix-ddr5",
    name: "SK hynix DDR5 RDIMM",
    slug: "sk-hynix-ddr5-rdimm",
    manufacturer: "SK hynix",
    manufacturerId: "sk-hynix",
    series: "DDR5 RDIMM",
    categoryId: "memory-ddr5",
    parentCategoryId: "ai-memory",
    categoryName: "DDR5 Memory",
    images: ["/images/server-room-1.jpg"],
    description:
      "Enterprise DDR5 RDIMM memory running at 5600MT/s for server and workstation platforms.",
    longDescription:
      "SK hynix DDR5 RDIMM delivers high-performance memory for enterprise servers and workstations at 5600MT/s. Featuring ECC and On-Die ECC for data integrity, it is available in 32GB, 64GB, and 128GB capacities with PMIC integration and 1.1V operation for reliable, power-efficient server memory deployments.",
    keyFeatures: [
      "5600 MT/s Speed",
      "ECC + On-Die ECC",
      "32GB / 64GB / 128GB",
      "PMIC Integration",
      "1.1V Low Power",
    ],
    useCases: ["virtualization", "database", "ai-inference"],
    bestFor: "Enterprise Server Memory",
    specs: {
      type: "DDR5 RDIMM",
      capacity: "32GB / 64GB / 128GB",
      speed: "5600 MT/s",
      formFactor: "288-pin RDIMM",
      voltage: "1.1V",
      bandwidth: "44.8 GB/s per module",
    },
    status: "in_stock",
    sortOrder: 2,
    isFeatured: false,
    isPopular: true,
    seo: {
      metaTitle: "SK hynix DDR5 RDIMM | Servchip",
      metaDescription:
        "Enterprise DDR5 RDIMM at 5600MT/s for server platforms.",
    },
    createdAt: "2023-09-01",
    updatedAt: "2024-06-01",
  },
  {
    id: "sk-hynix-hbm3",
    name: "SK hynix HBM3",
    slug: "sk-hynix-hbm3",
    manufacturer: "SK hynix",
    manufacturerId: "sk-hynix",
    series: "HBM3",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-2.jpg"],
    description:
      "High-bandwidth HBM3 memory for AI accelerators with 819 GB/s bandwidth per stack.",
    longDescription:
      "SK hynix HBM3 delivers 819 GB/s bandwidth per stack in 8-Hi and 12-Hi configurations, providing the high-bandwidth memory required by AI accelerators and HPC processors. Built on advanced TSV technology, it is used in NVIDIA H100 and other flagship accelerators for training and inference workloads.",
    keyFeatures: [
      "819 GB/s Bandwidth per Stack",
      "Up to 24GB Capacity",
      "8-Hi / 12-Hi Stacks",
      "TSV Technology",
      "1.0V Operation",
    ],
    useCases: ["ai-training", "ai-inference", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3",
      capacity: "24GB per stack",
      speed: "6.4 Gbps",
      formFactor: "8-Hi / 12-Hi Stack",
      voltage: "1.0V",
      bandwidth: "819 GB/s per stack",
    },
    status: "in_stock",
    sortOrder: 3,
    isFeatured: true,
    isPopular: false,
    seo: {
      metaTitle: "SK hynix HBM3 | Servchip",
      metaDescription:
        "HBM3 memory with 819 GB/s bandwidth for AI accelerators.",
    },
    createdAt: "2023-06-01",
    updatedAt: "2024-06-01",
  },
];

export const SAMSUNG_MEMORY: MemoryProduct[] = [
  {
    id: "samsung-hbm3e",
    name: "Samsung HBM3E",
    slug: "samsung-hbm3e",
    manufacturer: "Samsung",
    manufacturerId: "samsung",
    series: "HBM3E",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-2.jpg"],
    description:
      "Samsung HBM3E with 1.2 TB/s bandwidth for next-generation AI processors.",
    longDescription:
      "Samsung HBM3E memory delivers 1.2 TB/s bandwidth per stack with capacities up to 36GB. Built on Samsung's advanced fabrication technology, it powers the latest AI accelerators from leading vendors. Features include advanced thermal management and industry-leading power efficiency.",
    keyFeatures: [
      "1.2 TB/s Bandwidth",
      "36GB per Stack",
      "Advanced Thermal Management",
      "Low Power",
      "12-Hi Stack Support",
    ],
    useCases: ["ai-training", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3E",
      capacity: "36GB per stack",
      speed: "9.8 Gbps",
      formFactor: "8-Hi / 12-Hi Stack",
      voltage: "1.1V",
      bandwidth: "1.2 TB/s per stack",
    },
    status: "in_stock",
    sortOrder: 1,
    isFeatured: true,
    isPopular: true,
    seo: {
      metaTitle: "Samsung HBM3E | Servchip",
      metaDescription: "Samsung HBM3E memory for AI processors.",
    },
    createdAt: "2024-07-01",
    updatedAt: "2025-01-01",
  },
  {
    id: "samsung-ddr5-rdimm",
    name: "Samsung DDR5 RDIMM",
    slug: "samsung-ddr5-rdimm",
    manufacturer: "Samsung",
    manufacturerId: "samsung",
    series: "DDR5 RDIMM",
    categoryId: "memory-ddr5",
    parentCategoryId: "ai-memory",
    categoryName: "DDR5 Memory",
    images: ["/images/server-room-3.jpg"],
    description:
      "Enterprise DDR5 RDIMM for server platforms with up to 5600 Mbps speed.",
    longDescription:
      "Samsung DDR5 RDIMM delivers high-performance memory for enterprise servers and data centers. With speeds up to 5600 Mbps, ECC support, and On-Die ECC, it provides reliable operation for mission-critical workloads including virtualization, databases, and AI inference.",
    keyFeatures: [
      "5600 Mbps Speed",
      "ECC Support",
      "On-Die ECC",
      "Low Power",
      "32GB/64GB/128GB",
    ],
    useCases: ["virtualization", "data-analytics", "ai-inference"],
    bestFor: "Enterprise Server Memory",
    specs: {
      type: "DDR5 RDIMM",
      capacity: "32GB / 64GB / 128GB",
      speed: "5600 Mbps",
      formFactor: "288-pin RDIMM",
      voltage: "1.1V",
      bandwidth: "44.8 GB/s per module",
    },
    status: "in_stock",
    sortOrder: 2,
    isFeatured: false,
    isPopular: true,
    seo: {
      metaTitle: "Samsung DDR5 RDIMM | Servchip",
      metaDescription: "Enterprise DDR5 RDIMM for server platforms.",
    },
    createdAt: "2023-09-01",
    updatedAt: "2024-06-01",
  },
  {
    id: "samsung-hbm3",
    name: "Samsung HBM3",
    slug: "samsung-hbm3",
    manufacturer: "Samsung",
    manufacturerId: "samsung",
    series: "HBM3",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-3.jpg"],
    description:
      "Samsung HBM3 memory with 36GB capacity for AI accelerators and HPC platforms.",
    longDescription:
      "Samsung HBM3 delivers 36GB capacity per 12-Hi stack with 819 GB/s bandwidth, providing the memory density and bandwidth needed for advanced AI accelerators. Featuring Samsung's advanced TSV packaging and low-power design, it powers flagship AI training and inference platforms from major chip vendors.",
    keyFeatures: [
      "36GB per Stack",
      "819 GB/s Bandwidth",
      "12-Hi Stack",
      "Advanced TSV Packaging",
      "Low Power Design",
    ],
    useCases: ["ai-training", "ai-inference", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3",
      capacity: "36GB per stack",
      speed: "6.4 Gbps",
      formFactor: "12-Hi Stack",
      voltage: "1.0V",
      bandwidth: "819 GB/s per stack",
    },
    status: "in_stock",
    sortOrder: 3,
    isFeatured: true,
    isPopular: false,
    seo: {
      metaTitle: "Samsung HBM3 | Servchip",
      metaDescription: "Samsung HBM3 with 36GB for AI accelerators.",
    },
    createdAt: "2023-06-01",
    updatedAt: "2024-06-01",
  },
];

export const MICRON_MEMORY: MemoryProduct[] = [
  {
    id: "micron-hbm3e",
    name: "Micron HBM3E",
    slug: "micron-hbm3e",
    manufacturer: "Micron",
    manufacturerId: "micron",
    series: "HBM3E",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-4.png"],
    description:
      "Micron HBM3E with industry-leading 1.2+ TB/s bandwidth for AI and HPC.",
    longDescription:
      "Micron HBM3E memory delivers over 1.2 TB/s bandwidth per stack for the most demanding AI and HPC workloads. Available in 8-Hi and 12-Hi configurations, Micron's HBM3E offers the highest bandwidth density for AI accelerators in data centers worldwide.",
    keyFeatures: [
      "1.2+ TB/s Bandwidth",
      "Up to 36GB Stack",
      "Advanced Node",
      "Low Latency",
      "12-Hi Support",
    ],
    useCases: ["ai-training", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3E",
      capacity: "36GB per stack",
      speed: "9.6 Gbps",
      formFactor: "8-Hi / 12-Hi Stack",
      voltage: "1.1V",
      bandwidth: "1.2+ TB/s per stack",
    },
    status: "in_stock",
    sortOrder: 1,
    isFeatured: true,
    isPopular: true,
    seo: {
      metaTitle: "Micron HBM3E | Servchip",
      metaDescription: "Micron HBM3E for AI and HPC accelerators.",
    },
    createdAt: "2024-06-01",
    updatedAt: "2025-01-01",
  },
  {
    id: "micron-ddr5-rdimm",
    name: "Micron DDR5 RDIMM",
    slug: "micron-ddr5-rdimm",
    manufacturer: "Micron",
    manufacturerId: "micron",
    series: "DDR5",
    categoryId: "memory-ddr5",
    parentCategoryId: "ai-memory",
    categoryName: "DDR5 Memory",
    images: ["/images/server-room-5.jpg"],
    description: "Enterprise DDR5 RDIMM for next-generation server platforms.",
    longDescription:
      "Micron DDR5 RDIMM provides enterprise-grade memory performance and reliability for data center servers. With speeds up to 5600 MT/s and capacities from 16GB to 128GB, it supports the most demanding virtualization, database, and AI workloads with industry-leading power efficiency.",
    keyFeatures: [
      "5600 MT/s Speed",
      "ECC + On-Die ECC",
      "16GB-128GB",
      "1.1V Operation",
      "PMIC Integration",
    ],
    useCases: ["virtualization", "data-analytics"],
    bestFor: "Enterprise Memory Upgrade",
    specs: {
      type: "DDR5 RDIMM",
      capacity: "16GB / 32GB / 64GB / 128GB",
      speed: "5600 MT/s",
      formFactor: "288-pin RDIMM",
      voltage: "1.1V",
      bandwidth: "44.8 GB/s per module",
    },
    status: "in_stock",
    sortOrder: 2,
    isFeatured: false,
    isPopular: true,
    seo: {
      metaTitle: "Micron DDR5 RDIMM | Servchip",
      metaDescription: "Enterprise DDR5 RDIMM for next-gen servers.",
    },
    createdAt: "2023-09-01",
    updatedAt: "2024-06-01",
  },
  {
    id: "micron-hbm3",
    name: "Micron HBM3",
    slug: "micron-hbm3",
    manufacturer: "Micron",
    manufacturerId: "micron",
    series: "HBM3",
    categoryId: "memory-hbm",
    parentCategoryId: "ai-memory",
    categoryName: "AI Memory",
    images: ["/images/server-room-4.png"],
    description:
      "Micron HBM3 with 24GB capacity and 819 GB/s bandwidth for AI accelerator platforms.",
    longDescription:
      "Micron HBM3 delivers 24GB per stack with 819 GB/s bandwidth, offering high-performance memory for AI training and inference accelerators. Built on Micron's proven DRAM process with advanced TSV technology, it provides the reliable high-bandwidth memory required for next-generation AI and HPC workloads.",
    keyFeatures: [
      "24GB per Stack",
      "819 GB/s Bandwidth",
      "8-Hi / 12-Hi Stacks",
      "TSV Technology",
      "1.0V Operation",
    ],
    useCases: ["ai-training", "ai-inference", "hpc"],
    bestFor: "AI Accelerator Memory",
    specs: {
      type: "HBM3",
      capacity: "24GB per stack",
      speed: "6.4 Gbps",
      formFactor: "8-Hi / 12-Hi Stack",
      voltage: "1.0V",
      bandwidth: "819 GB/s per stack",
    },
    status: "in_stock",
    sortOrder: 3,
    isFeatured: false,
    isPopular: false,
    seo: {
      metaTitle: "Micron HBM3 | Servchip",
      metaDescription: "Micron HBM3 with 24GB for AI accelerator platforms.",
    },
    createdAt: "2023-06-01",
    updatedAt: "2024-06-01",
  },
];

export const MEMORY_MODULES: MemoryProduct[] = [
  {
    id: "mrdimm",
    name: "MRDIMM (Multiplexed Rank DIMM)",
    slug: "mrdimm",
    manufacturer: "JEDEC",
    manufacturerId: "jedec",
    series: "MRDIMM",
    categoryId: "memory-ddr5",
    parentCategoryId: "ai-memory",
    categoryName: "DDR5 Memory",
    images: ["/images/server-room-1.jpg"],
    description:
      "Next-generation multiplexed rank DIMM for 8800+ MT/s server memory performance.",
    longDescription:
      "MRDIMM is the next-generation server memory standard that multiplexes multiple DDR5 DIMM ranks for 8800+ MT/s effective speed. Designed for Intel Xeon 6 and future platforms, it doubles memory bandwidth without requiring new memory controller changes. Available in 128GB, 256GB, and higher capacities.",
    keyFeatures: [
      "8800+ MT/s",
      "Multiplexed Ranks",
      "Backward Compatible",
      "Up to 256GB",
      "Improved Bandwidth",
    ],
    useCases: ["hpc", "ai-training", "virtualization"],
    bestFor: "High-Performance Server Memory",
    specs: {
      type: "MRDIMM",
      capacity: "128GB / 256GB",
      speed: "8800+ MT/s",
      formFactor: "288-pin RDIMM",
      voltage: "1.1V",
      bandwidth: "70+ GB/s per module",
    },
    status: "pre_order",
    sortOrder: 1,
    isFeatured: true,
    isPopular: false,
    seo: {
      metaTitle: "MRDIMM | Servchip",
      metaDescription: "Next-gen multiplexed rank DIMM for 8800+ MT/s.",
    },
    createdAt: "2025-01-01",
    updatedAt: "2025-06-01",
  },
  {
    id: "cxl-memory-module",
    name: "CXL Memory Module",
    slug: "cxl-memory-module",
    manufacturer: "Samsung",
    manufacturerId: "samsung",
    series: "CXL",
    categoryId: "memory-cxl",
    parentCategoryId: "ai-memory",
    categoryName: "CXL Memory",
    images: ["/images/server-room-2.jpg"],
    description:
      "CXL-attached memory module for memory expansion beyond traditional DIMM slots.",
    longDescription:
      "CXL Memory Modules provide memory expansion and pooling for CXL-enabled servers. Based on the Compute Express Link standard, these modules allow servers to access far memory beyond their local DIMM capacity, enabling higher memory capacity for in-memory databases, AI training, and large-scale virtualization.",
    keyFeatures: [
      "CXL 2.0 Interface",
      "Memory Expansion",
      "Pooling Support",
      "Hot Plug",
      "Up to 512GB",
    ],
    useCases: ["virtualization", "data-analytics", "ai-training"],
    bestFor: "Server Memory Expansion",
    specs: {
      type: "CXL Memory Module",
      capacity: "256GB / 512GB",
      speed: "CXL 2.0 32GT/s",
      formFactor: "E3.S / EDSFF",
      voltage: "—",
      bandwidth: "64 GB/s per module",
    },
    status: "on_order",
    sortOrder: 1,
    isFeatured: false,
    isPopular: false,
    seo: {
      metaTitle: "CXL Memory Module | Servchip",
      metaDescription: "CXL-attached memory for server expansion.",
    },
    createdAt: "2024-09-01",
    updatedAt: "2025-01-01",
  },
];

export const ALL_MEMORY: MemoryProduct[] = [
  ...SK_HYNIX_MEMORY,
  ...SAMSUNG_MEMORY,
  ...MICRON_MEMORY,
  ...MEMORY_MODULES,
];
